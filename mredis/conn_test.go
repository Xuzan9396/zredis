...
func TestRedisPool_CommonCmd(t *testing.T) {
	...
	passwd := "27252725"
	dbnum := 0
	mredis.Conn("redis1", conn, passwd, dbnum, mredis.WithIdleTime(60*time.Second))

	mredis.CommonHset("redis1", "test_lua_hset", "coin", 100)
	mredis.CommonHset("redis1", "test_lua_hset", "coin_free", 10)
	resBytes, err := redis.Values(mredis.CommonLuaScript("redis1", GIFT_DRAW_POOL, "test_lua_hset", 2))
	mredis.CommonEXPIRE("redis1", "test_lua_hset", 3600)
	if err != nil {
		t.Error(err)
		return
	}
